...
def listar_pagos(pc_id: int, db: Session = Depends(get_db)):
    _get_pc(pc_id, db)
    return db.query(Pago).filter(Pago.pedido_cliente_id == pc_id).order_by(Pago.fecha).all()
...
def eliminar_pago(pc_id: int, pago_id: int, db: Session = Depends(get_db)):
    pago = db.query(Pago).filter(Pago.id == pago_id, Pago.pedido_cliente_id == pc_id).first()
    if not pago:
